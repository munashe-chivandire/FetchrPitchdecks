(function () {
  ...
  function animateCounters(slideEl) {
    slideEl.querySelectorAll('[data-count-to]').forEach(function (el) {
  ...
  function goTo(index, direction) {
    ...
    prev.querySelectorAll('[data-animate]').forEach(function (el) {
      el.classList.remove('animate-in');
      el.style.animationDelay = '';
    });
    setTimeout(function () { prev.classList.remove('exit-up', 'exit-down'); }, 600);
    ...
    requestAnimationFrame(function () {
      next.querySelectorAll('[data-animate]').forEach(function (el, i) {
  ...
  document.addEventListener('click', function (e) {
    if (e.target.closest('a,button,input,textarea,kbd,.nav-panel')) return;
  ...
  var RING_CIRCUMFERENCE = 113.1; // 2 * pi * 18
