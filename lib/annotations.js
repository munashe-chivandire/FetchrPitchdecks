...
(function () {
  ...
  function navigateToSlide(targetSlide) {
    ...
    const targetEl = document.querySelector('.slide[data-slide="' + targetSlide + '"]');
    if (!targetEl || targetEl === currentActive) return;

    const currentNum = currentActive ? parseInt(currentActive.dataset.slide, 10) : 0;
    const direction = targetSlide > currentNum ? 'down' : 'up';

    // Remove active + add exit class to current
    if (currentActive) {
      currentActive.classList.remove('active');
      currentActive.classList.add(direction === 'down' ? 'exit-up' : 'exit-down');
      setTimeout(function () {
        currentActive.classList.remove('exit-up', 'exit-down');
      }, 600);
    }
    ...
    if (progressBar) {
      ...
      progressBar.style.width = ((targetSlide / (total - 1)) * 100) + '%';
    }
    ...
    if (counter) {
      var total = slides.length;
      counter.textContent = (targetSlide + 1) + ' / ' + total;
    }

    // Trigger any data-animate elements on the new slide
    targetEl.querySelectorAll('[data-animate]').forEach(function (el) {
  ...
  // Expose resolve/delete for inline onclick handlers
